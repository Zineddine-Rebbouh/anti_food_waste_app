          ...
          children: [
            ...
            Center(
              child: Text(
                ...
                style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFFD1D5DB)),
              ),
            ),
            const SizedBox(height: 48),
          ],
  ...
  void _showChangePassword(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(l10n.change_password, style: const TextStyle(fontWeight: FontWeight.w800)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(decoration: InputDecoration(labelText: l10n.current_password, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            const SizedBox(height: 16),
            TextField(decoration: InputDecoration(labelText: l10n.new_password, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () { Navigator.pop(ctx); _showSnack(context, l10n.password_updated); },
  ...
  void _confirmDeactivate(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: Colors.white,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)),
        title: Text(l10n.deactivate_account_confirm, style: const TextStyle(fontWeight: FontWeight.w800)),
        content: Text(l10n.deactivate_warning),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text(l10n.cancel, style: const TextStyle(color: Colors.grey))),
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text(l10n.deactivate, style: const TextStyle(color: Color(0xFFEF4444), fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
...
class _LanguageTile extends StatelessWidget {
  ...
  const _LanguageTile({required this.flag, required this.label, required this.selected, required this.onTap, required String code});
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      title: Text(label, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: selected ? const Color(0xFF2D8659) : const Color(0xFF111827))),
      trailing: selected ? const Icon(Icons.check_circle, color: Color(0xFF2D8659), size: 22) : null,
    );
  }
}
...
class _SwitchTile extends StatelessWidget {
  ...
  const _SwitchTile({required this.icon, required this.iconColor, required this.label, required this.subtitle, required this.value, required this.onChanged});
...
class _ActionTile extends StatelessWidget {
  ...
  const _ActionTile({required this.icon, required this.iconColor, required this.label, required this.subtitle, required this.onTap, this.textColor});
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
      leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: iconColor.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: Icon(icon, color: iconColor, size: 20)),
      title: Text(label, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: textColor ?? const Color(0xFF111827))),
      subtitle: Text(subtitle, style: const TextStyle(fontSize: 12, color: Colors.grey)),
